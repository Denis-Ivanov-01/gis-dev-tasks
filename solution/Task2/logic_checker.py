...
class LogicChecker:
    ...
    def __init__(self, config: Config, geo_accessor: GeoDataAccessor):
        ...
        arcpy.env.workspace = config.database_path
    ...
    def get_guid(self, layer, where):
        logging.debug(f"Fetching GUID from layer '{layer}' with where-clause: {where}")
        filtered = arcpy.management.SelectLayerByAttribute(layer, where_clause=where)
        if self.get_layer_count(filtered) != 1:
